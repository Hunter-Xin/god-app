<template>
  <ul class="movieTop">
    <li class="movieTop-list" v-for="(obj,index) in menuList" :key="index" @click="goInner2(obj.id)">
...
export default {
  ...
  methods: {
      getList() {
        ...
        axios.get(`https://douban.uieee.com/v2/movie/top250?start=${this.menuList.length}&count=10`)
            .then( res => {
            console.log(res);
      ...
      goInner2(id) {
        this.$router.push('/movieInner2/'+id)
      }
  },
  created () {
      //接口调用
